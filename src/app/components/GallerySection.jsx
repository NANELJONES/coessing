...
const GallerySection = ({ schoolSlug }) => {
  ...
  if (galleries.length === 0 && !loading) {
    return (
      <div className="text-center py-12">
        <p className="text-white text-lg">No gallery content available for this school.</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-8">
      <h2 className="text-4xl font-bold text-white mb-8">Gallery</h2>
      ...
      <div className="space-y-12">
        {galleries.map((gallery, index) => (
          ...
          >
            <h3 className="text-2xl font-semibold text-white mb-4">{gallery.title}</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {gallery.galleryAsset && Array.isArray(gallery.galleryAsset) && gallery.galleryAsset.length > 0 ? (
                ...
                <div className="col-span-full text-center py-8">
                  <p className="text-white">No images available for this gallery</p>
                </div>
              )}
            </div>
          </div>
      ...
      {loading && (
        <div className="text-center py-8">
          ...
          <p className="text-white mt-2">Loading more galleries...</p>
        </div>
      )}

      {!hasNextPage && galleries.length > 0 && (
        <div className="text-center py-8">
          <p className="text-white text-lg">No more galleries to load.</p>
        </div>
      )}
